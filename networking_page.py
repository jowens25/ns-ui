...
async def interface_page(interface_name: str):    
    ...
    async def state_changed_cb(u):
        interface = await GetInterfaceData(nm, interface_name)
        interface_card.refresh(interface)

    nm.on_state_changed(state_changed_cb)


    @ui.refreshable
    async def interface_card(interface):
        with ui.card().classes("w-full"):
            with ui.row().classes("w-full items-center justify-between"):
                ...
                ui.switch("Connected").on('click', lambda e: connection_sw_cb(e)).props("flat color=accent").bind_value(interface, "Active")
                ...
                with ui.column().classes("flex-1 gap-4"):  # Fixed width for labels
                    ...
                    with ui.row().classes("flex-1 gap-16"):
                        ...
                        ui.checkbox('Connect automatically', on_change=auto_connect_cb).props("flat color=accent dense").bind_value(interface, "AutoConnect")
                    with ui.row().classes("flex-1 gap-16"):
                        ui.label("IPv4").classes("font-bold w-8")
                        ui.label().bind_text_from(interface, "Ip4")
                        ui.label("Edit").classes("text-accent cursor-pointer hover:underline").on('click', lambda: edit_ip_connection('ipv4', device))
                    with ui.row().classes("flex-1 gap-16"):
                        ui.label("IPv6").classes("font-bold w-8")
                        ui.label().bind_text_from(interface, "Ip6")
                        ui.label("Edit").classes("text-accent cursor-pointer hover:underline").on('click', lambda: edit_ip_connection('ipv6', device))
...
async def edit_ip_connection(version :str, device: ProxyInterface):
    ...
    async def ip_address_list():
        for addr in ip.AddressData:
            with ui.row():
                ...
                ui.input(label="Gateway").props("dense").classes("flex-1").bind_value(ip, "Gateway")
    ...
    async def dns_search_list():
        for search in ip.DnsSearch:
            with ui.row():
                ui.input(label="Server").props("dense").classes("flex-1").bind_value(search, "Server")
            ...
            with ui.column().classes("w-full"):
                ...
                with ui.row().classes("w-full justify-between"):  
                    ...
                    with ui.row():
                        ...
                        ui.select(options=["disabled", "auto", "manual"], on_change=on_method_change).props("dense").classes("w-24").bind_value(ip, "Method")
                ...
                with ui.row().classes("w-full justify-between"):
                    ...
                    with ui.row():
                        dns_server_switch = ui.switch("Manual").props("flat color=accent").props("dense").classes("w-24").bind_value(ip, "IgnoreAutoDns")
                ...
                with ui.row().classes("w-full justify-between"):
                    ui.label("Routes")
                    with ui.row():
                        route_switch = ui.switch("Manual").props("flat color=accent").props("dense").classes("w-24").bind_value(ip, "IgnoreAutoRoutes")
