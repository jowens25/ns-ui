...
import { Z as De, _ as parseYaml, at as Ze, ct as ze, et as J, lt as Vu, m as parseJson, nt as T, o as isVersionUnsupportedBelow, ut as require_picocolors } from "./core-B7rc0ndx.mjs";
...
var X = l((ke, B) => {
	...
	function Kt(t, e) {
		var n = t.mode, r = t.uid, s = t.gid, o = e.uid !== void 0 ? e.uid : process.getuid && process.getuid(), i = e.gid !== void 0 ? e.gid : process.getgid && process.getgid(), a = parseInt("100", 8), c = parseInt("010", 8), u = parseInt("001", 8), f = a | c;
		return n & u || n & c && s === i || n & a && r === o || n & f && o === 0;
	}
});
var U = l((Ae, G) => {
	"use strict";
...
var et = l((Re, tt) => {
	"use strict";
	var g = process.platform === "win32" || process.env.OSTYPE === "cygwin" || process.env.OSTYPE === "msys", Y = h("path"), Bt = g ? ";" : ":", V = U(), J$1 = (t) => Object.assign(/* @__PURE__ */ new Error(`not found: ${t}`), { code: "ENOENT" }), Q = (t, e) => {
		...
		let { pathEnv: r, pathExt: s, pathExtExe: o } = Q(t, e), i = [], a = (u) => new Promise((f, p) => {
			if (u === r.length) return e.all && i.length ? f(i) : p(J$1(t));
			let d = r[u], w = /^".*"$/.test(d) ? d.slice(1, -1) : d, m = Y.join(w, t);
			f(c(!w && /^\.[\\\/]/.test(t) ? t.slice(0, 2) + m : m, u, 0));
...
var bt = l((ze$1, vt) => {
	...
	function k(t, e) {
		return Object.assign(/* @__PURE__ */ new Error(`${e} ${t.command} ENOENT`), {
			code: "ENOENT",
			errno: "ENOENT",
			syscall: `${e} ${t.command}`,
			path: t.command,
...
var Ct = l((He, E) => {
	"use strict";
	var yt = h("child_process"), T$1 = wt(), A = bt();
	function _t(t, e, n) {
		let r = T$1(t, e, n), s = yt.spawn(r.command, r.args, r.options);
		return A.hookChildProcess(s, r), s;
	}
	function pe(t, e, n) {
		let r = T$1(t, e, n), s = yt.spawnSync(r.command, r.args, r.options);
...
function Ft(t, e) {
	...
	do
		n.push(resolve$1(r, "node_modules", ".bin")), s = r, r = dirname$1(r);
	while (r !== s);
	return {
		key: e.key,
...
function up$1(base, options) {
	...
	let tmp = absolute(base, cwd$1), root = !stop;
	let prev, arr = [];
...
async function formatFiles(options) {
	...
	try {
		...
		stop("Failed to format files");
		T.error(e?.output?.stderr || "unknown error");
		return;
	}
...
async function getPnpmVersion() {
	let v = void 0;
	try {
		v = (await be("pnpm", ["--version"], { throwOnError: true })).stdout.trim();
	} catch {}
	return v;
}

//#endregion
export { be as C, dist as D, templates as E, getSharedFiles as O, resolveCommand as S, create as T, writeFile as _, installOption as a, from as b, Option as c, fileExists as d, formatFiles as f, readFile as g, installPackages as h, installDependencies as i, program as l, getPackageJson as m, addPnpmBuildDependencies as n, packageManagerPrompt as o, getHighlighter as p, getUserAgent as r, Command as s, AGENT_NAMES as t, commonFilePaths as u, any as v, ve as w, detect as x, up as y };